...
function convertImageLinksToImgTags(text: string): string {
  ...
  const imgTag = '<div style="width: 400px;height: 250px;background-repeat: no-repeat;background-size: 100%;background-image: url($1);border-radius: 10px;margin-top: 15px;"></div>';
...
<template>
  <div class="main">
    <div v-for="data in store.mainView.message" class="messageView">
      ...
      <div class="message" v-html="convertImageLinksToImgTags(data.message)"></div>
    </div>
  </div>
</template>
